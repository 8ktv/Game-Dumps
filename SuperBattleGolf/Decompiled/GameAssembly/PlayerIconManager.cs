...
public class PlayerIconManager : SingletonBehaviour<PlayerIconManager>
{
	...
	{
		return GetPlayerIcon(steamId, size);
	}
	...
	public static Sprite GetPlayerIcon(ulong playerGuid, IconSize size)
	{
		...
		return SingletonBehaviour<PlayerIconManager>.Instance.GetPlayerIconInternal(playerGuid, size);
	}
	...
	private Sprite GetPlayerIconInternal(ulong steamId, IconSize size)
	{
		...
		async void LoadIconAsync()
		{
			...
			bool gotSteamFriend;
			Friend playerAsSteamFriend;
			do
			{
				gotSteamFriend = TryGetPlayerAsSteamFriend(steamId, out playerAsSteamFriend);
				await UniTask.WaitForSeconds(0.1f);
			}
			while (!gotSteamFriend && BMath.GetTimeSince(startTime) < 2f);
			if (!gotSteamFriend && loadedIcons.TryGetValue((size, steamId), out var value2))
			{
				UnityEngine.Object.Destroy(value2);
				loadedIcons.Remove((size, steamId));
			}
			else
			{
