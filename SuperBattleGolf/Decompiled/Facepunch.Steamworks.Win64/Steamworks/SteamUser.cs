...
public class SteamUser : SteamClientClass<SteamUser>
{
	...
	public static bool HasVoiceData
	{
		get
		{
			...
			if (Internal.GetAvailableVoice(ref pcbCompressed, ref pcbUncompressed_Deprecated, 0u) != VoiceResult.OK)
			{
				return false;
			}
	...
	public unsafe static int ReadVoiceData(Stream stream)
	{
		...
		{
			if (Internal.GetVoice(bWantCompressed: true, (IntPtr)ptr, (uint)readBuffer.Length, ref nBytesWritten, bWantUncompressed_Deprecated: false, IntPtr.Zero, 0u, ref nUncompressBytesWritten_Deprecated, 0u) != VoiceResult.OK)
			{
				return 0;
			}
		}
	...
	public unsafe static int DecompressVoice(Stream input, int length, Stream output)
	{
		...
		fixed (byte* ptr = array)
		{
			...
			{
				if (Internal.DecompressVoice((IntPtr)ptr, (uint)length, (IntPtr)ptr2, (uint)array2.Length, ref nBytesWritten, SampleRate) != VoiceResult.OK)
				{
					return 0;
				}
			}
	...
	public unsafe static int DecompressVoice(byte[] from, Stream output)
	{
		...
		fixed (byte* ptr = from)
		{
			...
			{
				if (Internal.DecompressVoice((IntPtr)ptr, (uint)from.Length, (IntPtr)ptr2, (uint)array.Length, ref nBytesWritten, SampleRate) != VoiceResult.OK)
				{
					return 0;
				}
			}
	...
	public static int DecompressVoice(IntPtr from, int length, IntPtr to, int bufferSize)
	{
		...
		uint nBytesWritten = 0u;
		if (Internal.DecompressVoice(from, (uint)length, to, (uint)bufferSize, ref nBytesWritten, SampleRate) != VoiceResult.OK)
		{
			return 0;
		}
		return (int)nBytesWritten;
