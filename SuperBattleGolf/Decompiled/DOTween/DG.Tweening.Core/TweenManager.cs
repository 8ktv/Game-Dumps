...
internal static class TweenManager
{
	...
	internal static void SetCapacities(int tweenersCapacity, int sequencesCapacity)
	{
		...
		{
			int num = tweenersCapacity;
			tweenersCapacity = 1;
			if (Debugger.logPriority >= 1)
			{
				Debugger.LogWarning(string.Format("SetTweensCapacity({0},{1}): can't set Tweeners capacity to less than 1 ► changed to ({2},{1})", num, sequencesCapacity, tweenersCapacity));
			}
		}
		if (sequencesCapacity < 1)
		{
			int num2 = sequencesCapacity;
			sequencesCapacity = 1;
			if (Debugger.logPriority >= 1)
			{
				Debugger.LogWarning(string.Format("SetTweensCapacity({0},{1}): can't set Sequences capacity to less than 1 ► changed to ({0},{2})", tweenersCapacity, num2, sequencesCapacity));
			}
		}
		if (tweenersCapacity < totActiveTweeners)
		{
			int num3 = tweenersCapacity;
			tweenersCapacity = totActiveTweeners;
			if (Debugger.logPriority >= 1)
			{
				Debugger.LogWarning(string.Format("SetTweensCapacity({0},{1}): can't set Tweeners capacity to less than current existing Tweeners ► changed to ({2},{1})", num3, sequencesCapacity, tweenersCapacity));
			}
		}
		if (sequencesCapacity < totActiveSequences)
		{
			int num4 = sequencesCapacity;
			sequencesCapacity = totActiveSequences;
			if (Debugger.logPriority >= 1)
			{
				Debugger.LogWarning(string.Format("SetTweensCapacity({0},{1}): can't set Sequences capacity to less than current existing Sequences ► changed to ({0},{2})", tweenersCapacity, num4, sequencesCapacity));
			}
		}
		if (tweenersCapacity < sequencesCapacity)
		{
			int num5 = tweenersCapacity;
			tweenersCapacity = sequencesCapacity;
			if (Debugger.logPriority >= 1)
			{
				Debugger.LogWarning(string.Format("SetTweensCapacity({0},{1}): Sequences capacity must be less or equal than Tweeners capacity ► changed to ({2},{1})", num5, sequencesCapacity, tweenersCapacity));
			}
		}
