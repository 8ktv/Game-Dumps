...
internal struct Transform
{
	...
	public Transform Inverse()
	{
		...
		m = math.inverse(m);
		float3 @float = -math.mul(m, new float3(row0.w, row1.w, row2.w));
		Transform result = default(Transform);
		result.row0 = new float4(m[0].x, m[1].x, m[2].x, @float.x);
		result.row1 = new float4(m[0].y, m[1].y, m[2].y, @float.y);
		result.row2 = new float4(m[0].z, m[1].z, m[2].z, @float.z);
		return result;
	}
}
