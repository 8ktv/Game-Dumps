		...
		for (int j = 0; j < m_characterCount; j++)
		{
			...
			case HorizontalAlignmentOptions.Flush:
			{
				...
				if ((!char.IsControl(character3) && lineNumber < m_lineNumber) || flag25 || tMP_LineInfo.maxAdvance > tMP_LineInfo.width)
				{
					...
					int num90 = tMP_LineInfo.visibleSpaceCount - tMP_LineInfo.controlCharacterCount;
					if (flag21)
					{
						num90--;
						num89++;
			...
			if (characterInfo[j].isVisible)
			{
				...
				case TMP_TextElementType.Character:
				{
					...
					num79 = characterInfo[j].scale * (1f - m_charWidthAdjDelta);
					...
					case RenderMode.ScreenSpaceOverlay:
						num79 *= Mathf.Abs(f) / scaleFactor;
						break;
					case RenderMode.ScreenSpaceCamera:
						num79 *= (flag23 ? Mathf.Abs(f) : 1f);
						break;
