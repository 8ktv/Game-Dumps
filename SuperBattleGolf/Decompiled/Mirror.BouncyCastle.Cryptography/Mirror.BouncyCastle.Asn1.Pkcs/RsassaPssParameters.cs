...
public class RsassaPssParameters : Asn1Encodable
{
	...
	public RsassaPssParameters(Asn1Sequence seq)
	{
		...
		for (int i = 0; i != seq.Count; i++)
		{
			...
			case 1:
				...
				break;
			case 2:
				saltLength = DerInteger.GetInstance(asn1TaggedObject, declaredExplicit: true);
				break;
			case 3:
				trailerField = DerInteger.GetInstance(asn1TaggedObject, declaredExplicit: true);
				break;
			default:
	...
	public override Asn1Object ToAsn1Object()
	{
		...
		{
			asn1EncodableVector.Add(new DerTaggedObject(isExplicit: true, 0, hashAlgorithm));
		}
		if (!maskGenAlgorithm.Equals(DefaultMaskGenFunction))
		{
			asn1EncodableVector.Add(new DerTaggedObject(isExplicit: true, 1, maskGenAlgorithm));
		}
		if (!saltLength.Equals(DefaultSaltLength))
		{
			asn1EncodableVector.Add(new DerTaggedObject(isExplicit: true, 2, saltLength));
		}
		if (!trailerField.Equals(DefaultTrailerField))
		{
			asn1EncodableVector.Add(new DerTaggedObject(isExplicit: true, 3, trailerField));
		}
