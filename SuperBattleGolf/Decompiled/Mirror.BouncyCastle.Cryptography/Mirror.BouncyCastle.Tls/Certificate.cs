...
public sealed class Certificate
{
	...
	public void Encode(TlsContext context, Stream messageOutput, Stream endPointHashOutput)
	{
		...
		List<byte[]> list2 = (flag ? new List<byte[]>(num) : null);
		long num2 = 0L;
		for (int i = 0; i < num; i++)
		{
		...
		for (int j = 0; j < num; j++)
		{
			TlsUtilities.WriteOpaque24(list[j], messageOutput);
			if (flag)
			{
				TlsUtilities.WriteOpaque16(list2[j], messageOutput);
			}
		}
	}
	...
	public static Certificate Parse(ParseOptions options, TlsContext context, Stream messageInput, Stream endPointHashOutput)
	{
		...
		if (num == 0)
		{
			if (flag)
			{
				if (array.Length >= 1)
				{
					return new Certificate(certificateType, array, EmptyCertEntries);
				}
				return EmptyChainTls13;
			}
			return EmptyChain;
		}
		byte[] array2 = TlsUtilities.ReadFully(num, messageInput);
		MemoryStream memoryStream = new MemoryStream(array2, writable: false);
		TlsCrypto crypto = context.Crypto;
		int num2 = System.Math.Max(1, options.MaxChainLength);
		List<CertificateEntry> list = new List<CertificateEntry>();
		while (memoryStream.Position < memoryStream.Length)
		{
			...
			byte[] encoding;
			if (flag || certificateType != 2)
			{
				encoding = TlsUtilities.ReadOpaque24(memoryStream, 1);
			}
			else
			{
				encoding = array2;
			...
			TlsCertificate tlsCertificate = crypto.CreateCertificate(certificateType, encoding);
		...
		CertificateEntry[] array3 = new CertificateEntry[list.Count];
		for (int i = 0; i < list.Count; i++)
		{
			array3[i] = list[i];
		}
		return new Certificate(certificateType, array, array3);
	}
