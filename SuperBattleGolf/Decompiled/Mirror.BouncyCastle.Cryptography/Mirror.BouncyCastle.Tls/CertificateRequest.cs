...
public sealed class CertificateRequest
{
	...
	private static IList<SignatureAndHashAlgorithm> CheckSupportedSignatureAlgorithms(IList<SignatureAndHashAlgorithm> supportedSignatureAlgorithms, short alertDescription)
	{
	...
	private CertificateRequest(byte[] certificateRequestContext, short[] certificateTypes, IList<SignatureAndHashAlgorithm> supportedSignatureAlgorithms, IList<SignatureAndHashAlgorithm> supportedSignatureAlgorithmsCert, IList<X509Name> certificateAuthorities)
	{
	...
	public void Encode(TlsContext context, Stream output)
	{
		...
		bool flag2 = TlsUtilities.IsTlsV13(serverVersion);
		if (flag2 != (m_certificateRequestContext != null) || flag2 != (m_certificateTypes == null) || flag != (m_supportedSignatureAlgorithms != null) || (!flag2 && m_supportedSignatureAlgorithmsCert != null))
		{
		...
		List<byte[]> list = new List<byte[]>(m_certificateAuthorities.Count);
		int num = 0;
	...
	public static CertificateRequest Parse(TlsContext context, Stream input)
	{
		...
		if (TlsUtilities.IsTlsV13(serverVersion))
		{
			...
			IDictionary<int, byte[]> extensions = TlsProtocol.ReadExtensionsData13(13, extBytes);
			IList<SignatureAndHashAlgorithm> supportedSignatureAlgorithms = CheckSupportedSignatureAlgorithms(TlsExtensionsUtilities.GetSignatureAlgorithmsExtension(extensions), 109);
			IList<SignatureAndHashAlgorithm> signatureAlgorithmsCertExtension = TlsExtensionsUtilities.GetSignatureAlgorithmsCertExtension(extensions);
			IList<X509Name> certificateAuthoritiesExtension = TlsExtensionsUtilities.GetCertificateAuthoritiesExtension(extensions);
