...
public abstract class TlsProtocol : TlsCloseable
{
			...
			while (count > 0)
			{
				...
				if (m_appDataSplitEnabled)
				{
					...
					default:
						if (count > 1)
						{
							SafeWriteRecord(23, buffer, offset, 1);
							offset++;
							count--;
						}
						break;
				...
				int num = System.Math.Min(count, m_recordStream.PlaintextLimit);
				SafeWriteRecord(23, buffer, offset, num);
				offset += num;
				count -= num;
			}
		}
	...
	protected virtual void ProcessFinishedMessage(MemoryStream buf)
	{
		...
		if (!Arrays.FixedTimeEquals(array, b))
		{
	...
	internal static void WritePreSharedKeyExtension(MemoryStream buf, IDictionary<int, byte[]> extensions, int bindersSize)
	{
		if (extensions.TryGetValue(41, out var value))
		{
