...
internal sealed class RecordStream
{
	...
	internal RecordPreview PreviewRecordHeader(byte[] recordHeader)
	{
		...
		if (23 == num && m_handler.IsApplicationDataReady)
		{
			...
			contentLimit = System.Math.Max(0, System.Math.Min(val2: (!(readCipher is TlsCipherExt tlsCipherExt)) ? readCipher.GetPlaintextLimit(num2) : tlsCipherExt.GetPlaintextDecodeLimit(num2), val1: m_plaintextLimit));
		}
