...
public abstract class Symmetric
{
	...
	internal class AesSymmetric : Symmetric
	{
		...
		internal override void Prf(byte[] output, byte[] input, int inLen, int outputLen)
		{
			ParametersWithIV parameters = new ParametersWithIV(new KeyParameter(input, 0, inLen), new byte[16]);
			cipher.Init(forEncryption: true, parameters);
			byte[] input2 = new byte[outputLen];
			for (int i = 0; i < outputLen; i += 16)
			{
				cipher.ProcessBlock(input2, i, output, i);
			}
		}
	}
