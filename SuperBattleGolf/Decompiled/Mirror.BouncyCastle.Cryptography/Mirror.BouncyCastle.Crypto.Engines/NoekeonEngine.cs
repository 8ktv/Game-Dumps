...
public class NoekeonEngine : IBlockCipher
{
	...
	public virtual void Init(bool forEncryption, ICipherParameters parameters)
	{
		...
		if (!forEncryption)
		{
			...
			uint num6 = num2 ^ num4;
			num6 ^= Integers.RotateLeft(num6, 8) ^ Integers.RotateLeft(num6, 24);
			num ^= num6;
			num2 ^= num5;
			num3 ^= num6;
			num4 ^= num5;
	...
	private int EncryptBlock(byte[] input, int inOff, byte[] output, int outOff)
	{
		...
		while (true)
		{
			...
			num3 ^= num7;
			num4 ^= num8;
			uint num11 = num2 ^ num4;
			num11 ^= Integers.RotateLeft(num11, 8) ^ Integers.RotateLeft(num11, 24);
			num ^= num11;
			num2 ^= num10;
			num3 ^= num11;
			num4 ^= num10;
