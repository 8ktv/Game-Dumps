...
public class CamelliaLightEngine : IBlockCipher
{
	...
	private int ProcessBlock128(byte[] input, int inOff, byte[] output, int outOff)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			array[i] = Pack.BE_To_UInt32(input, inOff + i * 4) ^ kw[i];
		}
		...
		Pack.UInt32_To_BE(array[2] ^ kw[4], output, outOff);
		Pack.UInt32_To_BE(array[3] ^ kw[5], output, outOff + 4);
		Pack.UInt32_To_BE(array[0] ^ kw[6], output, outOff + 8);
		Pack.UInt32_To_BE(array[1] ^ kw[7], output, outOff + 12);
		return 16;
	}
