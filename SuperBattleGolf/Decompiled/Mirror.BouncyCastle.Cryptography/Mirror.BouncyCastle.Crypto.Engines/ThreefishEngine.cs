...
public class ThreefishEngine : IBlockCipher
{
	...
	public virtual void Init(bool forEncryption, ICipherParameters parameters)
	{
		...
		if (key != null)
		{
			...
			{
				int num = blocksizeBytes;
				throw new ArgumentException("Threefish key must be same size as block (" + num + " bytes)");
			}
	...
	private void SetKey(ulong[] key)
	{
		...
		{
			int num = blocksizeWords;
			throw new ArgumentException("Threefish key must be same size as block (" + num + " words)");
		}
		ulong num2 = 2004413935125273122uL;
		for (int i = 0; i < blocksizeWords; i++)
		{
			kw[i] = key[i];
			num2 ^= kw[i];
		}
		kw[blocksizeWords] = num2;
