...
internal class GFCalculator
{
	...
	{
		return Utils.ToUnsigned16Bits(Utils.ToUnsigned16Bits(-a >> 31) & Utils.ToUnsigned16Bits(-b >> 31) & exp[mod(log[a] + log[b])]);
	}

	internal static int mod(int a)
	{
		int num = Utils.ToUnsigned16Bits(a - 255);
		int num2 = Utils.ToUnsigned8bits(-(num >> 15));
		return Utils.ToUnsigned16Bits(num + (num2 & 0xFF));
	}

	internal static int inverse(int a)
	{
		return Utils.ToUnsigned16Bits(-a >> 31) & exp[255 - log[a]];
	}
}
