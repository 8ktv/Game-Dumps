...
public sealed class PostProcessLayer : MonoBehaviour
{
	...
	private void UpdateBundleSortList(List<SerializedBundleRef> sortedList, PostProcessEvent evt)
	{
		...
		{
			string typeName = item2.settings.GetType().AssemblyQualifiedName;
		...
		foreach (SerializedBundleRef sorted in sortedList)
		{
			...
			PostProcessBundle bundle = effects.Find((PostProcessBundle b) => b.settings.GetType().AssemblyQualifiedName == typeName2);
		...
		if (m_CurrentContext.IsTemporalAntialiasingActive())
		{
			if (!m_Camera.usePhysicalProperties)
			{
				...
				if (m_Camera.stereoEnabled)
				{
					...
					{
						m_Camera.CopyStereoDeviceProjectionMatrixToNonJittered(Camera.StereoscopicEye.Left);
	...
	private void BuildCommandBuffers()
	{
		...
		RenderTextureFormat renderTextureFormat = (m_Camera.targetTexture ? m_Camera.targetTexture.format : (m_Camera.allowHDR ? RuntimeUtilities.defaultHDRRenderTextureFormat : RenderTextureFormat.Default));
		...
		bool flag4 = settings.IsEnabledAndSupported(currentContext);
		if (currentContext.stereoActive)
		{
			currentContext.UpdateSinglePassStereoState(currentContext.IsTemporalAntialiasingActive(), flag, flag4);
		}
		if (num)
		{
		...
		bool flag5 = fog.IsEnabledAndSupported(currentContext);
		bool flag6 = HasOpaqueOnlyEffects(currentContext);
		int num2 = 0;
		num2 += (flag4 ? 1 : 0);
		num2 += (flag5 ? 1 : 0);
		num2 += (flag6 ? 1 : 0);
		...
		if (num2 > 0)
		{
			...
			int src = -1;
			int dst = -1;
			UpdateSrcDstForOpaqueOnly(ref src, ref dst, currentContext, renderTargetIdentifier, num2 + 1);
			if (RequiresInitialBlit(m_Camera, currentContext) || num2 == 1)
			{
				...
				UpdateSrcDstForOpaqueOnly(ref src, ref dst, currentContext, renderTargetIdentifier, num2);
			}
			if (flag4)
			{
		...
		int num3 = -1;
		bool flag7 = !m_NaNKilled && stopNaNPropagation && RuntimeUtilities.isFloatingPointFormat(renderTextureFormat);
		if ((!currentContext.stereoActive || currentContext.numberOfEyes <= 1 || currentContext.stereoRenderingMode != PostProcessRenderContext.StereoRenderingMode.SinglePassInstanced) && (RequiresInitialBlit(m_Camera, currentContext) || flag7))
		{
			...
			currentContext.GetScreenSpaceTemporaryRT(m_LegacyCmdBuffer, num3, 0, renderTextureFormat, RenderTextureReadWrite.sRGB, FilterMode.Bilinear, width);
			m_LegacyCmdBuffer.BuiltinBlit(renderTargetIdentifier, num3, RuntimeUtilities.copyStdMaterial, stopNaNPropagation ? 1 : 0);
			if (!m_NaNKilled)
			{
		...
		if (num3 > -1)
		{
			m_LegacyCmdBuffer.ReleaseTemporaryRT(num3);
		}
	}
		...
		for (int i = 0; i < context.numberOfEyes; i++)
		{
			...
			if (stopNaNPropagation && !m_NaNKilled)
			{
				...
				if (context.stereoActive && context.numberOfEyes > 1)
				{
					...
					{
						command.BlitFullscreenTriangleFromTexArray(context.source, num, RuntimeUtilities.copyFromTexArraySheet, 1, clear: false, i);
						flag = true;
					}
	...
	private void RenderList(List<SerializedBundleRef> list, PostProcessRenderContext context, string marker)
	{
		...
		else
		{
			...
			for (int k = 0; k < count; k++)
			{
				...
				m_ActiveEffects[k].RenderOrLog(context);
			}
			command.ReleaseTemporaryRT(num);
			if (count > 2)
			{
