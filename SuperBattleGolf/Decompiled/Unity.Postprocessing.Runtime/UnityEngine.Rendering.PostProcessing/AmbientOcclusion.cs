...
public sealed class AmbientOcclusion : PostProcessEffectSettings
{
	...
	public override bool IsEnabledAndSupported(PostProcessRenderContext context)
	{
		...
		else if (mode.value == AmbientOcclusionMode.MultiScaleVolumetricObscurance)
		{
			...
			flag &= SystemInfo.supportsComputeShaders && !RuntimeUtilities.isOpenGLES && !RuntimeUtilities.isWebNonWebGPU && SystemInfo.IsFormatSupported(GraphicsFormat.R32_SFloat, GraphicsFormatUsage.Render) && SystemInfo.IsFormatSupported(GraphicsFormat.R16_SFloat, GraphicsFormatUsage.Render) && SystemInfo.IsFormatSupported(GraphicsFormat.R8_UNorm, GraphicsFormatUsage.Render);
		}
		return flag;
	}
}
