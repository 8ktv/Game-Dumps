...
public class TlsClientProtocol : TlsProtocol
{
	...
	protected override void HandleHandshakeMessage(short type, HandshakeMessageInput buf)
	{
		...
		case 11:
		{
			...
			if (connectionState == 4 || connectionState == 6)
			{
				...
				m_authentication = TlsUtilities.ReceiveServerCertificate(m_tlsClientContext, m_tlsClient, buf, m_serverExtensions);
				m_connectionState = 7;
				break;
			}
			throw new TlsFatalAlert(10);
		...
		case 14:
			...
			case 11:
			{
				...
				if (m_certificateRequest != null)
				{
					tlsCredentials = TlsUtilities.EstablishClientCredentials(m_authentication, m_certificateRequest);
					if (tlsCredentials != null)
					{
						...
						if (tlsCredentials is TlsCredentialedSigner)
						{
							...
							if (ProtocolVersion.TLSv12.Equals(securityParameters.NegotiatedVersion))
							{
								...
								{
									TlsUtilities.TrackHashAlgorithmClient(m_handshakeHash, signatureAndHashAlgorithm);
								}
							}
							if (tlsStreamSigner != null)
							{
				...
				if (tlsCredentialedSigner != null)
				{
					DigitallySigned certificateVerify = TlsUtilities.GenerateCertificateVerifyClient(m_tlsClientContext, tlsCredentialedSigner, signatureAndHashAlgorithm, tlsStreamSigner, m_handshakeHash);
	...
	protected virtual void Process13ServerHello(ServerHello serverHello, bool afterHelloRetryRequest)
	{
		...
		securityParameters.m_statusRequestVersion = (m_clientExtensions.ContainsKey(5) ? 1 : 0);
		...
		else
		{
			...
			if (!m_clientAgreements.TryGetValue(keyShareServerHello.NamedGroup, out var value))
			{
				throw new TlsFatalAlert(47);
			}
			value.ReceivePeerValue(keyShareServerHello.KeyExchange);
			sharedSecret = value.CalculateSecret();
		}
		m_clientAgreements = null;
		m_clientBinders = null;
	...
	protected virtual void ProcessServerHello(ServerHello serverHello)
	{
		...
		securityParameters.m_resumedSession = array.Length != 0 && m_tlsSession != null && Arrays.AreEqual(array, m_tlsSession.SessionID);
		if (securityParameters.IsResumedSession && (serverHello.CipherSuite != m_sessionParameters.CipherSuite || !securityParameters.NegotiatedVersion.Equals(m_sessionParameters.NegotiatedVersion)))
		{
		...
		if (TlsExtensionsUtilities.HasExtendedMasterSecretExtension(m_clientExtensions))
		{
			...
			{
				if (!flag && m_tlsClient.RequiresExtendedMasterSecret())
				{
		...
		if (securityParameters.IsResumedSession && securityParameters.IsExtendedMasterSecret != m_sessionParameters.IsExtendedMasterSecret)
		{
		...
		IDictionary<int, byte[]> dictionary = m_clientExtensions;
	...
	protected virtual void Receive13EncryptedExtensions(MemoryStream buf)
	{
		...
		SecurityParameters securityParameters = m_tlsClientContext.SecurityParameters;
		_ = securityParameters.NegotiatedVersion;
		securityParameters.m_applicationProtocol = TlsExtensionsUtilities.GetAlpnExtensionServer(m_serverExtensions);
		securityParameters.m_applicationProtocolSet = true;
		IDictionary<int, byte[]> dictionary = m_clientExtensions;
		IDictionary<int, byte[]> serverExtensions = m_serverExtensions;
		if (securityParameters.IsResumedSession)
		{
			dictionary = null;
			serverExtensions = m_sessionParameters.ReadServerExtensions();
		}
		securityParameters.m_maxFragmentLength = TlsUtilities.ProcessMaxFragmentLengthExtension(dictionary, serverExtensions, 47);
		securityParameters.m_encryptThenMac = false;
		securityParameters.m_truncatedHmac = false;
		if (!securityParameters.IsResumedSession)
		{
			securityParameters.m_statusRequestVersion = (m_clientExtensions.ContainsKey(5) ? 1 : 0);
			securityParameters.m_clientCertificateType = TlsUtilities.ProcessClientCertificateTypeExtension13(dictionary, serverExtensions, 47);
			securityParameters.m_serverCertificateType = TlsUtilities.ProcessServerCertificateTypeExtension13(dictionary, serverExtensions, 47);
		}
	...
	protected virtual void Receive13ServerCertificate(MemoryStream buf)
	{
		...
		m_authentication = TlsUtilities.Receive13ServerCertificate(m_tlsClientContext, m_tlsClient, buf, m_serverExtensions);
		HandleServerCertificate();
	}
	...
	{
		IDictionary<int, byte[]> extensions = m_clientHello.Extensions;
