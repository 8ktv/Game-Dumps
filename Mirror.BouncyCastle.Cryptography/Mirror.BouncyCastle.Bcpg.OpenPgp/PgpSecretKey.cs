...
public class PgpSecretKey : PgpObject
{
	...
	internal PgpSecretKey(PgpPrivateKey privKey, PgpPublicKey pubKey, SymmetricKeyAlgorithmTag encAlgorithm, byte[] rawPassPhrase, bool clearPassPhrase, bool useSha1, SecureRandom rand, bool isMasterKey)
	{
		...
		{
			if (privKey.Key is ECPrivateKeyParameters eCPrivateKeyParameters)
			{
			...
			bcpgObject = new ECSecretBcpgKey(new BigInteger(1, x25519PrivateKeyParameters.GetEncoded(), bigEndian: false));
			break;
		}
		case PublicKeyAlgorithmTag.ECDsa:
			...
			break;
		case PublicKeyAlgorithmTag.EdDsa:
			if (privKey.Key is Ed25519PrivateKeyParameters ed25519PrivateKeyParameters)
			{
				bcpgObject = new EdSecretBcpgKey(new BigInteger(1, ed25519PrivateKeyParameters.GetEncoded()));
				break;
			}
			if (privKey.Key is Ed448PrivateKeyParameters ed448PrivateKeyParameters)
			{
	...
	internal PgpPrivateKey DoExtractPrivateKey(byte[] rawPassPhrase, bool clearPassPhrase)
	{
		...
		try
		{
			...
			case PublicKeyAlgorithmTag.ECDH:
			{
				...
				privateKey = ((!EdECObjectIdentifiers.id_X25519.Equals(curveOid2) && !CryptlibObjectIdentifiers.curvey25519.Equals(curveOid2)) ? ((!EdECObjectIdentifiers.id_X448.Equals(curveOid2)) ? new ECPrivateKeyParameters("ECDH", eCSecretBcpgKey2.X, eCDHPublicBcpgKey.CurveOid) : PrivateKeyFactory.CreateKey(new PrivateKeyInfo(new AlgorithmIdentifier(curveOid2), new DerOctetString(Arrays.ReverseInPlace(BigIntegers.AsUnsignedByteArray(eCSecretBcpgKey2.X)))))) : PrivateKeyFactory.CreateKey(new PrivateKeyInfo(new AlgorithmIdentifier(curveOid2), new DerOctetString(Arrays.ReverseInPlace(BigIntegers.AsUnsignedByteArray(eCSecretBcpgKey2.X))))));
				break;
			}
			case PublicKeyAlgorithmTag.ECDsa:
			{
	...
	public void Encode(Stream outStr)
	{
		...
		if (pub.subSigs == null)
		{
			...
			{
				IUserDataPacket userDataPacket = pub.ids[i];
				if (userDataPacket is UserIdPacket p)
				{
					bcpgOutputStream.WritePacket(p);
				}
				else
				{
					if (!(userDataPacket is PgpUserAttributeSubpacketVector pgpUserAttributeSubpacketVector))
					{
						throw new InvalidOperationException();
					}
					bcpgOutputStream.WritePacket(new UserAttributePacket(pgpUserAttributeSubpacketVector.ToSubpacketArray()));
				}
				TrustPacket trustPacket = pub.idTrusts[i];
				if (trustPacket != null)
				{
					bcpgOutputStream.WritePacket(trustPacket);
				}
				foreach (PgpSignature item in pub.idSigs[i])
				{
	...
	private static byte[] GetDValue(Stream inputStream, byte[] rawPassPhrase, bool clearPassPhrase, string curveName)
	{
		...
		if (SXprUtilities.ReadString(inputStream, inputStream.ReadByte()).Equals("protected"))
		{
			...
			MemoryStream memoryStream = new MemoryStream(RecoverKeyData(SymmetricKeyAlgorithmTag.Aes128, "/CBC/NoPadding", key, iv, array, 0, array.Length), writable: false);
