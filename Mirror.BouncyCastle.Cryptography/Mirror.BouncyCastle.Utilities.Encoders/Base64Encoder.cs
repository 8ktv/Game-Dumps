...
public class Base64Encoder : IEncoder
{
	...
	public int Decode(byte[] data, int off, int length, Stream outStream)
	{
		...
		int num4 = num3 - 4;
		int num5;
		for (num5 = NextI(data, off, num4); num5 < num4; num5 = NextI(data, num5, num4))
		{
		...
		int num6 = NextI(data, num5, num3);
		int num7 = NextI(data, num6 + 1, num3);
		int num8 = NextI(data, num7 + 1, num3);
		int num9 = NextI(data, num8 + 1, num3);
		return num2 + DecodeLastBlock(outStream, (char)data[num6], (char)data[num7], (char)data[num8], (char)data[num9]);
	}
	...
	public int DecodeString(string data, Stream outStream)
	{
		...
		int num3 = num2 - 4;
		int num4;
		for (num4 = NextI(data, 0, num3); num4 < num3; num4 = NextI(data, num4, num3))
		{
			byte b = decodingTable[(uint)data[num4++]];
			num4 = NextI(data, num4, num3);
			byte b2 = decodingTable[(uint)data[num4++]];
			num4 = NextI(data, num4, num3);
			byte b3 = decodingTable[(uint)data[num4++]];
			num4 = NextI(data, num4, num3);
			byte b4 = decodingTable[(uint)data[num4++]];
			if ((b | b2 | b3 | b4) >= 128)
			{
				throw new IOException("invalid characters encountered in base64 data");
			}
