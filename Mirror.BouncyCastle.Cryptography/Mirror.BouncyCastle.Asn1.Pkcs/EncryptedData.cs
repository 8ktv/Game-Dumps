...
public class EncryptedData : Asn1Encodable
{
	...
	public Asn1OctetString Content
	{
		get
		{
			if (data.Count == 3)
			{
				return Asn1OctetString.GetInstance((Asn1TaggedObject)data[2], declaredExplicit: false);
			}
			return null;
		}
	}
	...
	private EncryptedData(Asn1Sequence seq)
	{
		...
		if (!((DerInteger)seq[0]).HasValue(0))
		{
	...
	{
		data = new BerSequence(contentType, encryptionAlgorithm.ToAsn1Object(), new BerTaggedObject(isExplicit: false, 0, content));
	}
