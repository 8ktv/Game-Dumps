...
internal static class Nat
{
	...
	public static uint ShiftUpBit(int len, uint[] z, int zOff, uint c)
	{
		...
		{
			uint num2 = z[zOff + i];
			uint num3 = z[zOff + i + 1];
			uint num4 = z[zOff + i + 2];
			uint num5 = z[zOff + i + 3];
			z[zOff + i] = (num2 << 1) | (c >> 31);
			z[zOff + i + 1] = (num3 << 1) | (num2 >> 31);
			z[zOff + i + 2] = (num4 << 1) | (num3 >> 31);
			z[zOff + i + 3] = (num5 << 1) | (num4 >> 31);
			c = num5;
		}
		for (; i < len; i++)
		{
			uint num6 = z[zOff + i];
			z[zOff + i] = (num6 << 1) | (c >> 31);
			c = num6;
		}
		return c >> 31;
	}
	...
	public static uint ShiftUpBit(int len, uint[] x, int xOff, uint c, uint[] z, int zOff)
	{
		...
		{
			uint num2 = x[xOff + i];
			uint num3 = x[xOff + i + 1];
			uint num4 = x[xOff + i + 2];
			uint num5 = x[xOff + i + 3];
			z[zOff + i] = (num2 << 1) | (c >> 31);
			z[zOff + i + 1] = (num3 << 1) | (num2 >> 31);
			z[zOff + i + 2] = (num4 << 1) | (num3 >> 31);
			z[zOff + i + 3] = (num5 << 1) | (num4 >> 31);
	...
	public static ulong ShiftUpBit64(int len, ulong[] x, ulong c, ulong[] z)
	{
		...
		for (int num = len - 4; i <= num; i += 4)
		{
			...
			ulong num5 = x[i + 3];
			z[i] = (num2 << 1) | (c >> 63);
			z[i + 1] = (num3 << 1) | (num2 >> 63);
			z[i + 2] = (num4 << 1) | (num3 >> 63);
			z[i + 3] = (num5 << 1) | (num4 >> 63);
			c = num5;
		}
		for (; i < len; i++)
		{
