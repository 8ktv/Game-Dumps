...
public abstract class AbstractF2mPoint : ECPointBase
{
	...
	protected override bool SatisfiesOrder()
	{
		...
		{
			ECFieldElement affineXCoord = Normalize().AffineXCoord;
			return ((AbstractF2mFieldElement)affineXCoord).Trace() != 0;
		}
		if (BigInteger.ValueOf(4).Equals(cofactor))
		{
			...
			return ((AbstractF2mFieldElement)eCFieldElement2).Trace() == 0;
		}
	...
	public override ECPoint ScaleX(ECFieldElement scale)
	{
		...
		case 5:
		{
			...
			return Curve.CreateRawPoint(rawXCoord2, y2, base.RawZCoords);
		}
		case 6:
		{
			ECFieldElement rawXCoord = base.RawXCoord;
			...
			return Curve.CreateRawPoint(rawXCoord, y, new ECFieldElement[1] { eCFieldElement2 });
		}
		default:
	...
	public override ECPoint ScaleY(ECFieldElement scale)
	{
		...
		if ((uint)(curveCoordinateSystem - 5) <= 1u)
		{
			...
			return Curve.CreateRawPoint(rawXCoord, y, base.RawZCoords);
		}
	...
	public virtual AbstractF2mPoint Tau()
	{
		...
		{
			ECFieldElement rawYCoord2 = base.RawYCoord;
			return (AbstractF2mPoint)curve.CreateRawPoint(rawXCoord.Square(), rawYCoord2.Square());
		}
		case 1:
		case 6:
		{
			ECFieldElement rawYCoord = base.RawYCoord;
			ECFieldElement eCFieldElement = base.RawZCoords[0];
			return (AbstractF2mPoint)curve.CreateRawPoint(rawXCoord.Square(), rawYCoord.Square(), new ECFieldElement[1] { eCFieldElement.Square() });
		}
		default:
			throw new InvalidOperationException("unsupported coordinate system");
		}
	...
	public virtual AbstractF2mPoint TauPow(int pow)
	{
		...
		{
			ECFieldElement rawYCoord2 = base.RawYCoord;
			return (AbstractF2mPoint)curve.CreateRawPoint(rawXCoord.SquarePow(pow), rawYCoord2.SquarePow(pow));
		}
		case 1:
		case 6:
		{
			ECFieldElement rawYCoord = base.RawYCoord;
			ECFieldElement eCFieldElement = base.RawZCoords[0];
			return (AbstractF2mPoint)curve.CreateRawPoint(rawXCoord.SquarePow(pow), rawYCoord.SquarePow(pow), new ECFieldElement[1] { eCFieldElement.SquarePow(pow) });
		}
		default:
			throw new InvalidOperationException("unsupported coordinate system");
		}
