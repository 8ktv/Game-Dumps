...
public class FpFieldElement : AbstractFpFieldElement
{
	...
	public override ECFieldElement Sqrt()
	{
		...
		BigInteger other = q.Subtract(BigInteger.One);
		while (true)
		{
			BigInteger bigInteger6 = BigInteger.Arbitrary(q.BitLength);
			if (bigInteger6.CompareTo(q) < 0 && ModReduce(bigInteger6.Multiply(bigInteger6).Subtract(bigInteger5)).ModPow(bigInteger3, q).Equals(other))
			{
				...
				if (!bigInteger7.Equals(BigInteger.One) && !bigInteger7.Equals(other))
				{
					break;
				}
			}
