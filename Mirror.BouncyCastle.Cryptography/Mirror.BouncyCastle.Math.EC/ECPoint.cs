...
public abstract class ECPoint
{
	private class ValidityCallback : IPreCompCallback
	{
		...
		{
			ValidityPreCompInfo validityPreCompInfo = (existing as ValidityPreCompInfo) ?? new ValidityPreCompInfo();
	...
	public virtual ECPoint ScaleX(ECFieldElement scale)
	{
		if (!IsInfinity)
		{
			return Curve.CreateRawPoint(RawXCoord.Multiply(scale), RawYCoord, RawZCoords);
		}
		return this;
	}
	...
	public virtual ECPoint ScaleXNegateY(ECFieldElement scale)
	{
		if (!IsInfinity)
		{
			return Curve.CreateRawPoint(RawXCoord.Multiply(scale), RawYCoord.Negate(), RawZCoords);
		}
		return this;
	}
	...
	public virtual ECPoint ScaleY(ECFieldElement scale)
	{
		if (!IsInfinity)
		{
			return Curve.CreateRawPoint(RawXCoord, RawYCoord.Multiply(scale), RawZCoords);
		}
		return this;
	}
	...
	public virtual ECPoint ScaleYNegateX(ECFieldElement scale)
	{
		if (!IsInfinity)
		{
			return Curve.CreateRawPoint(RawXCoord.Negate(), RawYCoord.Multiply(scale), RawZCoords);
		}
		return this;
	}
	...
	{
		return GetEncoded(compressed: false);
	}
