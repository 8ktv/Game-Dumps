...
{
	private static readonly byte[] RC2Ekb = new byte[256]
	{
	...
	private static BufferedBlockCipher CreateCipher(DerObjectIdentifier algorithm)
	{
		...
		{
			cipherMode = new CbcBlockCipher(AesUtilities.CreateEngine());
		}
		else if (PkcsObjectIdentifiers.DesEde3Cbc.Equals(algorithm))
		{
			cipherMode = new CbcBlockCipher(new DesEdeEngine());
		}
		else if (OiwObjectIdentifiers.DesCbc.Equals(algorithm))
		{
			cipherMode = new CbcBlockCipher(new DesEngine());
		}
		else if (PkcsObjectIdentifiers.RC2Cbc.Equals(algorithm))
		{
			cipherMode = new CbcBlockCipher(new RC2Engine());
		}
		else
		{
			...
			cipherMode = new CbcBlockCipher(new Cast5Engine());
		}
		return new PaddedBufferedBlockCipher(cipherMode, new Pkcs7Padding());
	}
}
