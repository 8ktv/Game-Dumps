...
public class DefaultDigestAlgorithmFinder : IDigestAlgorithmFinder
{
	...
	private static readonly HashSet<DerObjectIdentifier> Shake256Oids;
	...
	static DefaultDigestAlgorithmFinder()
	{
		...
		DigestNameToOids = new Dictionary<string, DerObjectIdentifier>(StringComparer.OrdinalIgnoreCase);
		DigestOidToAlgIDs = new Dictionary<DerObjectIdentifier, AlgorithmIdentifier>();
		Shake256Oids = new HashSet<DerObjectIdentifier>();
		...
		AddDigestAlgID(OiwObjectIdentifiers.IdSha1, withNullParams: true);
		AddDigestAlgID(NistObjectIdentifiers.IdSha224, withNullParams: false);
		AddDigestAlgID(NistObjectIdentifiers.IdSha256, withNullParams: false);
		AddDigestAlgID(NistObjectIdentifiers.IdSha384, withNullParams: false);
		AddDigestAlgID(NistObjectIdentifiers.IdSha512, withNullParams: false);
		AddDigestAlgID(NistObjectIdentifiers.IdSha512_224, withNullParams: false);
		AddDigestAlgID(NistObjectIdentifiers.IdSha512_256, withNullParams: false);
		AddDigestAlgID(NistObjectIdentifiers.IdSha3_224, withNullParams: false);
		AddDigestAlgID(NistObjectIdentifiers.IdSha3_256, withNullParams: false);
		AddDigestAlgID(NistObjectIdentifiers.IdSha3_384, withNullParams: false);
		AddDigestAlgID(NistObjectIdentifiers.IdSha3_512, withNullParams: false);
		AddDigestAlgID(NistObjectIdentifiers.IdShake128, withNullParams: false);
		AddDigestAlgID(NistObjectIdentifiers.IdShake256, withNullParams: false);
		AddDigestAlgID(CryptoProObjectIdentifiers.GostR3411, withNullParams: true);
		AddDigestAlgID(RosstandartObjectIdentifiers.id_tc26_gost_3411_12_256, withNullParams: false);
		AddDigestAlgID(RosstandartObjectIdentifiers.id_tc26_gost_3411_12_512, withNullParams: false);
		AddDigestAlgID(PkcsObjectIdentifiers.MD2, withNullParams: true);
		AddDigestAlgID(PkcsObjectIdentifiers.MD4, withNullParams: true);
		AddDigestAlgID(PkcsObjectIdentifiers.MD5, withNullParams: true);
		AddDigestAlgID(TeleTrusTObjectIdentifiers.RipeMD128, withNullParams: true);
		AddDigestAlgID(TeleTrusTObjectIdentifiers.RipeMD160, withNullParams: true);
		AddDigestAlgID(TeleTrusTObjectIdentifiers.RipeMD256, withNullParams: true);
	...
	{
		DerObjectIdentifier algorithm = signatureAlgorithm.Algorithm;
		if (Shake256Oids.Contains(algorithm))
		{
			return new AlgorithmIdentifier(NistObjectIdentifiers.IdShake256Len, new DerInteger(512));
		}
		DerObjectIdentifier digestOid = (PkcsObjectIdentifiers.IdRsassaPss.Equals(algorithm) ? RsassaPssParameters.GetInstance(signatureAlgorithm.Parameters).HashAlgorithm.Algorithm : (EdECObjectIdentifiers.id_Ed25519.Equals(algorithm) ? NistObjectIdentifiers.IdSha512 : ((!PkcsObjectIdentifiers.IdAlgHssLmsHashsig.Equals(algorithm)) ? CollectionUtilities.GetValueOrNull(DigestOids, algorithm) : NistObjectIdentifiers.IdSha256)));
	...
	public virtual AlgorithmIdentifier Find(DerObjectIdentifier digestOid)
	{
		if (digestOid == null)
		{
			throw new ArgumentNullException("digestOid");
		}
		if (DigestOidToAlgIDs.TryGetValue(digestOid, out var value))
		{
			return value;
		}
	...
	{
		if (DigestNameToOids.TryGetValue(digestName, out var value) || DerObjectIdentifier.TryFromID(digestName, out value))
		{
			return Find(value);
		}
		return null;
