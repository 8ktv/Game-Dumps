...
internal static class SecT233Field
{
	...
	{
		ulong num = Interleave.Unshuffle(x[0], x[1], out var even);
		...
		{
			int num4 = array2[i] >> 6;
			int num5 = array2[i] & 0x3F;
			array[num4] ^= num << num5;
			array[num4 + 1] ^= (num2 << num5) | (num >> -num5);
			array[num4 + 2] ^= (num3 << num5) | (num2 >> -num5);
			array[num4 + 3] ^= num3 >> -num5;
		}
		Reduce(array, z);
		z[0] ^= even;
		z[1] ^= even2;
