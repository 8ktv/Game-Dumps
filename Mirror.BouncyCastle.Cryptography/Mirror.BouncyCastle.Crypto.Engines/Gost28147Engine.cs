...
public class Gost28147Engine : IBlockCipher
{
	...
	static Gost28147Engine()
	{
		...
		m_sBoxes = new Dictionary<string, byte[]>(StringComparer.OrdinalIgnoreCase);
	...
	{
		int num = (int)Pack.LE_To_UInt32(inBytes, inOff);
	...
	{
		if (!m_sBoxes.TryGetValue(sBoxName, out var value))
		{
	...
	{
		foreach (KeyValuePair<string, byte[]> sBox2 in m_sBoxes)
		{
			if (Arrays.AreEqual(sBox2.Value, sBox))
			{
				return sBox2.Key;
			}
		}
