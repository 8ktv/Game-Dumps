...
public class Dstu7624WrapEngine : IWrapper
{
	...
	public byte[] Wrap(byte[] input, int inOff, int length)
	{
		...
		for (int i = 0; i < num2; i++)
		{
			...
			Array.Copy(list[0], 0, array, blockSize / 2, blockSize / 2);
			...
			for (int k = 2; k < num; k++)
			{
				Array.Copy(list[k - 1], 0, list[k - 2], 0, blockSize / 2);
			}
			Array.Copy(array, 0, list[num - 2], 0, blockSize / 2);
		}
		Array.Copy(array2, 0, array, 0, blockSize / 2);
		num4 = blockSize / 2;
		for (int l = 0; l < num - 1; l++)
		{
			Array.Copy(list[l], 0, array, num4, blockSize / 2);
			num4 += blockSize / 2;
		}
		return array;
	}
	...
	public byte[] Unwrap(byte[] input, int inOff, int length)
	{
		...
		for (int i = 0; i < num2; i++)
		{
			Array.Copy(list[num - 2], 0, array, 0, blockSize / 2);
			...
			for (int k = 2; k < num; k++)
			{
				Array.Copy(list[num - k - 1], 0, list[num - k], 0, blockSize / 2);
			}
			Array.Copy(array, blockSize / 2, list[0], 0, blockSize / 2);
		}
		Array.Copy(array2, 0, array, 0, blockSize / 2);
		num4 = blockSize / 2;
		for (int l = 0; l < num - 1; l++)
