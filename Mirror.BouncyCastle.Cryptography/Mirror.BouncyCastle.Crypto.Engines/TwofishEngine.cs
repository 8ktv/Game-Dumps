...
public sealed class TwofishEngine : IBlockCipher
{
	...
	private void SetKey(byte[] key)
	{
		...
		{
			int num = i * 8;
			array[i] = (int)Pack.LE_To_UInt32(key, num);
			array2[i] = (int)Pack.LE_To_UInt32(key, num + 4);
		...
		for (int j = 0; j < 20; j++)
		{
			...
			int i2 = F32(num2 + 16843009, array2);
			i2 = Integers.RotateLeft(i2, 8);
			num3 += i2;
			gSubKeys[j * 2] = num3;
			num3 += i2;
			gSubKeys[j * 2 + 1] = Integers.RotateLeft(num3, 9);
		}
		int x = array3[0];
		int x2 = array3[1];
		int x3 = array3[2];
		...
		for (int k = 0; k < 256; k++)
		{
			...
			int num6;
			int num7;
			int num4 = (num5 = (num6 = (num7 = k)));
			switch (k64Cnt & 3)
			{
			case 1:
				gSBox[k * 2] = gMDS0[(P[0, num4] & 0xFF) ^ M_b0(x)];
				gSBox[k * 2 + 1] = gMDS1[(P[0, num5] & 0xFF) ^ M_b1(x)];
				gSBox[k * 2 + 512] = gMDS2[(P[1, num6] & 0xFF) ^ M_b2(x)];
				gSBox[k * 2 + 513] = gMDS3[(P[1, num7] & 0xFF) ^ M_b3(x)];
				continue;
			case 0:
				num4 = (P[1, num4] & 0xFF) ^ M_b0(x4);
				num5 = (P[0, num5] & 0xFF) ^ M_b1(x4);
				num6 = (P[0, num6] & 0xFF) ^ M_b2(x4);
				num7 = (P[1, num7] & 0xFF) ^ M_b3(x4);
				goto case 3;
			case 3:
				num4 = (P[1, num4] & 0xFF) ^ M_b0(x3);
				num5 = (P[1, num5] & 0xFF) ^ M_b1(x3);
				num6 = (P[0, num6] & 0xFF) ^ M_b2(x3);
				num7 = (P[0, num7] & 0xFF) ^ M_b3(x3);
				break;
			case 2:
				break;
			default:
			...
			gSBox[k * 2] = gMDS0[(P[0, (P[0, num4] & 0xFF) ^ M_b0(x2)] & 0xFF) ^ M_b0(x)];
			gSBox[k * 2 + 1] = gMDS1[(P[0, (P[1, num5] & 0xFF) ^ M_b1(x2)] & 0xFF) ^ M_b1(x)];
			gSBox[k * 2 + 512] = gMDS2[(P[1, (P[0, num6] & 0xFF) ^ M_b2(x2)] & 0xFF) ^ M_b2(x)];
			gSBox[k * 2 + 513] = gMDS3[(P[1, (P[1, num7] & 0xFF) ^ M_b3(x2)] & 0xFF) ^ M_b3(x)];
		}
	}

	private void EncryptBlock(byte[] src, int srcIndex, byte[] dst, int dstIndex)
	{
		int num = (int)Pack.LE_To_UInt32(src, srcIndex) ^ gSubKeys[0];
		int num2 = (int)Pack.LE_To_UInt32(src, srcIndex + 4) ^ gSubKeys[1];
		int num3 = (int)Pack.LE_To_UInt32(src, srcIndex + 8) ^ gSubKeys[2];
		int num4 = (int)Pack.LE_To_UInt32(src, srcIndex + 12) ^ gSubKeys[3];
		int num5 = 8;
		for (int i = 0; i < 16; i += 2)
		{
			...
			num3 = Integers.RotateRight(num3, 1);
			num4 = Integers.RotateLeft(num4, 1) ^ (num6 + 2 * num7 + gSubKeys[num5++]);
			num6 = Fe32_0(num3);
			num7 = Fe32_3(num4);
			num ^= num6 + num7 + gSubKeys[num5++];
			num = Integers.RotateRight(num, 1);
			num2 = Integers.RotateLeft(num2, 1) ^ (num6 + 2 * num7 + gSubKeys[num5++]);
		}
		Pack.UInt32_To_LE((uint)(num3 ^ gSubKeys[4]), dst, dstIndex);
		Pack.UInt32_To_LE((uint)(num4 ^ gSubKeys[5]), dst, dstIndex + 4);
		Pack.UInt32_To_LE((uint)(num ^ gSubKeys[6]), dst, dstIndex + 8);
		Pack.UInt32_To_LE((uint)(num2 ^ gSubKeys[7]), dst, dstIndex + 12);
	}

	private void DecryptBlock(byte[] src, int srcIndex, byte[] dst, int dstIndex)
	{
		int num = (int)Pack.LE_To_UInt32(src, srcIndex) ^ gSubKeys[4];
		int num2 = (int)Pack.LE_To_UInt32(src, srcIndex + 4) ^ gSubKeys[5];
		int num3 = (int)Pack.LE_To_UInt32(src, srcIndex + 8) ^ gSubKeys[6];
		int num4 = (int)Pack.LE_To_UInt32(src, srcIndex + 12) ^ gSubKeys[7];
		int num5 = 39;
		for (int i = 0; i < 16; i += 2)
		{
			int num6 = Fe32_0(num);
			...
			num3 = Integers.RotateLeft(num3, 1) ^ (num6 + num7 + gSubKeys[num5--]);
			num4 = Integers.RotateRight(num4, 1);
			num6 = Fe32_0(num3);
			num7 = Fe32_3(num4);
			num2 ^= num6 + 2 * num7 + gSubKeys[num5--];
			num = Integers.RotateLeft(num, 1) ^ (num6 + num7 + gSubKeys[num5--]);
			num2 = Integers.RotateRight(num2, 1);
		}
		Pack.UInt32_To_LE((uint)(num3 ^ gSubKeys[0]), dst, dstIndex);
		Pack.UInt32_To_LE((uint)(num4 ^ gSubKeys[1]), dst, dstIndex + 4);
		Pack.UInt32_To_LE((uint)(num ^ gSubKeys[2]), dst, dstIndex + 8);
		Pack.UInt32_To_LE((uint)(num2 ^ gSubKeys[3]), dst, dstIndex + 12);
	}
