...
public abstract class SerpentEngineBase : IBlockCipher
{
	...
	protected void LT()
	{
		int num = Integers.RotateLeft(X0, 13);
		int num2 = Integers.RotateLeft(X2, 3);
		int i = X1 ^ num ^ num2;
		int i2 = X3 ^ num2 ^ (num << 3);
		X1 = Integers.RotateLeft(i, 1);
		X3 = Integers.RotateLeft(i2, 7);
		X0 = Integers.RotateLeft(num ^ X1 ^ X3, 5);
		X2 = Integers.RotateLeft(num2 ^ X3 ^ (X1 << 7), 22);
	}

	protected void InverseLT()
	{
		int num = Integers.RotateRight(X2, 22) ^ X3 ^ (X1 << 7);
		int num2 = Integers.RotateRight(X0, 5) ^ X1 ^ X3;
		int num3 = Integers.RotateRight(X3, 7);
		int num4 = Integers.RotateRight(X1, 1);
		X3 = num3 ^ num ^ (num2 << 3);
		X1 = num4 ^ num2 ^ num;
		X2 = Integers.RotateRight(num, 3);
		X0 = Integers.RotateRight(num2, 13);
	}
