...
public class PrivateKeyUsagePeriod : Asn1Encodable
{
	...
	public Asn1GeneralizedTime NotBefore => _notBefore;

	public Asn1GeneralizedTime NotAfter => _notAfter;
	...
	private PrivateKeyUsagePeriod(Asn1Sequence seq)
	{
		foreach (Asn1TaggedObject item in seq)
		{
			if (item.TagNo == 0)
			{
				_notBefore = Asn1GeneralizedTime.GetInstance(item, declaredExplicit: false);
			}
			else if (item.TagNo == 1)
			{
				_notAfter = Asn1GeneralizedTime.GetInstance(item, declaredExplicit: false);
			}
		}
	}
