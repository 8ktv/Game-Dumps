using System;
using Mirror.BouncyCastle.Math;
using Mirror.BouncyCastle.Utilities;

namespace Mirror.BouncyCastle.Crypto.Parameters;

public class RsaKeyParameters : AsymmetricKeyParameter
{
	private static readonly BigInteger SmallPrimesProduct = new BigInteger("", 16);

	private readonly BigInteger modulus;

	private readonly BigInteger exponent;

	public BigInteger Modulus => modulus;

	public BigInteger Exponent => exponent;

	private static bool HasAnySmallFactors(BigInteger modulus)
	{
		BigInteger m = modulus;
		BigInteger x = SmallPrimesProduct;
		if (modulus.BitLength < SmallPrimesProduct.BitLength)
		{
			m = SmallPrimesProduct;
			x = modulus;
		}
		return !BigIntegers.ModOddIsCoprimeVar(m, x);
	}

	private static BigInteger Validate(BigInteger modulus)
	{
		if ((modulus.IntValue & 1) == 0)
		{
			throw new ArgumentException("RSA modulus is even", "modulus");
		}
		int num = ImplGetInteger("Mirror.BouncyCastle.Rsa.MaxSize", 16384);
		if (modulus.BitLength > num)
		{
			throw new ArgumentException("RSA modulus out of range", "modulus");
		}
		if (HasAnySmallFactors(modulus))
		{
			throw new ArgumentException("RSA modulus has a small prime factor", "modulus");
		}
		return modulus;
	}

	public RsaKeyParameters(bool isPrivate, BigInteger modulus, BigInteger exponent)
		: base(isPrivate)
	{
		if (modulus == null)
		{
			throw new ArgumentNullException("modulus");
		}
		if (exponent == null)
		{
			throw new ArgumentNullException("exponent");
		}
		if (modulus.SignValue <= 0)
		{
			throw new ArgumentException("Not a valid RSA modulus", "modulus");
		}
		if (exponent.SignValue <= 0)
		{
			throw new ArgumentException("Not a valid RSA exponent", "exponent");
		}
		if (!isPrivate && (exponent.IntValue & 1) == 0)
		{
			throw new ArgumentException("RSA publicExponent is even", "exponent");
		}
		this.modulus = Validate(modulus);
		this.exponent = exponent;
	}

	public override bool Equals(object obj)
	{
		if (!(obj is RsaKeyParameters rsaKeyParameters))
		{
			return false;
		}
		if (rsaKeyParameters.IsPrivate == base.IsPrivate && rsaKeyParameters.Modulus.Equals(modulus))
		{
			return rsaKeyParameters.Exponent.Equals(exponent);
		}
		return false;
	}

	public override int GetHashCode()
	{
		return modulus.GetHashCode() ^ exponent.GetHashCode() ^ base.IsPrivate.GetHashCode();
	}

	private static int ImplGetInteger(string envVariable, int defaultValue)
	{
		if (!int.TryParse(Platform.GetEnvironmentVariable(envVariable), out var result))
		{
			return defaultValue;
		}
		return result;
	}
}
