...
public class KdfDoublePipelineIterationParameters : IDerivationParameters
{
	...
	public byte[] Ki => Arrays.Clone(ki);

	public bool UseCounter => useCounter;

	public int R => r;

	public byte[] FixedInputData => Arrays.Clone(fixedInputData);

	private KdfDoublePipelineIterationParameters(byte[] ki, byte[] fixedInputData, int r, bool useCounter)
	{
		if (ki == null)
		{
			throw new ArgumentNullException("A KDF requires Ki (a seed) as input", "ki");
		}
	...
	{
		return new KdfDoublePipelineIterationParameters(ki, fixedInputData, r, useCounter: true);
	}

	public static KdfDoublePipelineIterationParameters CreateWithoutCounter(byte[] ki, byte[] fixedInputData)
	{
		return new KdfDoublePipelineIterationParameters(ki, fixedInputData, UNUSED_R, useCounter: false);
	}
}
