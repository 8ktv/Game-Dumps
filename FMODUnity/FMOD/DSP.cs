...
public struct DSP
{
	...
	{
		return FMOD5_DSP_AddInput(handle, input.handle, out connection.handle, DSPCONNECTION_TYPE.PREALLOCATED);
	}
	...
	{
		return FMOD5_DSP_SetCallback(handle, callback);
	}
	...
	{
		return FMOD5_DSP_SetParameterData(handle, index, data, (data != null) ? ((uint)data.Length) : 0u);
	}
	...
	{
		IntPtr desc2;
		RESULT result = FMOD5_DSP_GetParameterInfo(handle, index, out desc2);
		desc = Marshal.PtrToStructure<DSP_PARAMETER_DESC>(desc2);
		return result;
	}
