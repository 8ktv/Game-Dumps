...
public class SingletonNetworkBehaviour<T> : SingletonNetworkBehaviourBase where T : NetworkBehaviour
{
	...
	private void EnsureSingleton()
	{
		...
		void SetAsInstance()
		{
			if (!(this is T val))
			{
				Debug.LogError("The generic type supplied to the SingletonNetworkBehaviour " + string.Format("class ({0}) does not match the instance's actual type ({1}) on {2}.", "T", GetType(), base.name));
			}
			else
			{
				instance = val;
