...
public sealed class PostProcessManager
{
	...
	{
		CleanBaseTypes();
		foreach (Type item in from t in RuntimeUtilities.GetAllTypesDerivedFrom<PostProcessEffectSettings>()
			where t.IsDefined(typeof(PostProcessAttribute), inherit: false) && !t.IsAbstract
			select t)
		{
